...
Run from pyRevit or RevitPythonShell:
  ...
  - inside a PROJECT: writes every placed instance of any family that carries
    ...
    the family's planes in family coordinates. Obstacles are drawn in the tool
    itself, so nothing else is exported.
...
def export():
    data = {'units': 'mm', 'source': doc.PathName, 'document': doc.Title, 'families': []}
