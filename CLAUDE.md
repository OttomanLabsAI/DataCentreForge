...
| v2.24 | Static models stay as modelled, dynamic ones work round them | Every sub-model can now be placed static or dynamic. A static model keeps its manholes where they are and its conduit banks on their modelled routes, never re-routed, and every dynamic run keeps clear of them; a dynamic model is routed and moved by the tool as before. Any mix of static and dynamic models can share one drawing. |
| v2.25 | Conduit runs get their encasement boxes | A conduit spec now carries an encasement offset, and every run on it wears a rectangular box that far beyond its conduits — as wide as the widest row, as high as all the rows. The plan, the elevation and the 3D view draw the box, the run reports its size, and other runs and obstacles keep their clearance from the box rather than the conduits. MV has an encasement from the start. |
| v2.26 | Layers like worksets, and an Encase button | Every chamber, run and obstacle now sits on a layer, listed in a new Layers window where each can be static or dynamic, shown or hidden, and one Update routes button fixes every static layer as it stands and routes every dynamic layer afresh round it. An Encase button in Create puts the encasement box on the selected runs, or all of them, and other runs now avoid that box itself. Ctrl with shift drags a manhole along its own axes. |
| v2.27 | A models window, and static models get out of the way | A Models window lists every model with a tick to hide all of it and four more for its manholes, conduits, encasement and obstacles on their own, and a Select button that takes the whole model in one press. Everything on a static model — manholes, conduits and encasement alike — is now something the dynamic runs route around, shown or hidden, and the MV model arrives with its encasement already modelled in, drawn as a solid box in three dimensions. |
| v2.28 | The spacing comes to the array panel | A run's array panel now carries the centres it is laid on, across and down, beside the picture of its section. They are the same numbers the spec and the manholes already held, so either place sets them and both show the change. |
